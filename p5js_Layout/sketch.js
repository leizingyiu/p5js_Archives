let titleFont, describeFont, fillC = 255, layout, layoutArguments, overLayout, autoSwitch,
...
let varNameArray = ['theText', 'overText'];
varNameArray.map(v => {
...
history.pushState('', '', url.origin + url.pathname + '?' + ['theText', 'overText'].map(t => `${t}=${eval(t)}`).join('&'));//前两个参数可省略
