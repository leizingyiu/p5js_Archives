/**
	...
	function setup(){
		keyFunc = new KeyboardCtrl(keyFuncSetup, holdFrame = 60, keySeparator = '_');
	}
	...
	Type:
		...
		once
			press to run one time 

		runToEnd
			press to run , hold to stop ,or return false to stop

		pressAgainToStop
			press to run , press again to stop

		pressReverseStopGoReverse
			press to run , press again to stop
			press to run and stop the return keyName 
		...
		pressAgainToAccelerateHoldToStop
			press to run , press again to accelerate
			hold and return false to stop

		pressAgainToLoopHoldToStop
			press to run , press again to mark the keyCount,then loop
			hold or return false to stop
		}

	KeyName{
		modifiedKeyName + keySeparator + keyCode
		shift_49
		[shift] + [1]
	}
