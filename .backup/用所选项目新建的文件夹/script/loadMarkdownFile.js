...
mdPath = typeof mdPath != 'undefined' && mdPath ? mdPath : (language == 'en' ? enMdPath : cnMdPath);
...
function addBackToCatalog() {
    if (document.querySelectorAll('#目录,#directory,#Directory').length == 0) {
        return;
    }
...
function loadReadmeFile(callback = (p) => { }) {
    ...
    p.id = 'readme' + String(Math.random()).replace(/\./, ''),
        ...
        p.setAttribute('data-mdfile', mdPath);
