...
let discriptText = {
	cn: `生成立体点阵，方便制作海报等背景使用

按 ctrl/cmd  隐藏点阵，保留方块
按 alt/opt 停止转动 
按 shift 隐藏方块，保留点阵
按 + - 可增减数量
按 f 可进入/退出全屏
按 t 隐藏/显示文本
按 s 保存图片`, en: `Generate a three-dimensional lattice, which is convenient for making posters and other backgrounds

Press ctrl/cmd to hide the dot matrix and keep the squares
Press alt/opt to stop turning
Press shift to hide the square, keep the lattice
Press + - to increase or decrease the quantity
Press f to enter/exit full screen
Press t to hide/show text
Press s to save the picture`}[language];
typoShow = typo;
