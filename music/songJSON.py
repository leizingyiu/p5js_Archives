...
for filepath, dirnames, filenames in os.walk(source_path):
    for filename in filenames:
        if(re.compile(r'(/\.)|(backup)').search(os.path.relpath(
                filepath, os.path.relpath(source_path, source_path))) == None):
            ...
            if(last_name in song_format_list):
                if(file_name not in musics):
                    musics[file_name] = {}
                # musics[file_name]['song'] = os.path.join('..', os.path.relpath(
                #     filepath, os.path.relpath(source_path, source_path)), filename)
                # songs.append(os.path.join('..', os.path.relpath(
                #     filepath, os.path.relpath(source_path, source_path)), filename))
                musics[file_name]['song'] = os.path.join(
                    'https://media.leizingyiu.net/music/', filename)
                songs.append(os.path.join(
                    'https://media.leizingyiu.net/music/', filename))
            if(last_name in lrc_format_list):
                if(file_name not in musics):
                    musics[file_name] = {}
                # musics[file_name]['lrc'] = os.path.join('..', os.path.relpath(
                #     filepath, os.path.relpath(source_path, source_path)), filename)
                musics[file_name]['lrc'] = os.path.join(
                    'https://media.leizingyiu.net/music/', filename)
